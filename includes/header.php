...
<head>
  ...
  <style id="hellobotz-floating-pill-navbar-style">
    ...
    /* PRODUCTS MEGA MENU: EXACT 1:1 CLONE OF GetGabs (media_1789590827070.png) */
    .mega-menu-products {
    ...
    .prodocu-mega-layout {
      display: grid !important;
      grid-template-columns: 145px 1.15fr 1.1fr 1.1fr 210px !important;
      gap: 20px !important;
      align-items: stretch !important;
      width: 100% !important;
    }
    ...
    .prodocu-channel-btn.active .prodocu-chan-icon {
      color: #034737 !important;
    }
    .prodocu-chan-icon {
    ...
    .prodocu-mob-channel-panel {
      padding: 8px 14px 14px 18px !important;
      margin-bottom: 8px !important;
    ...
    <div class="header-inner">
      ...
      <nav class="nav-desktop" role="navigation" aria-label="Main">
        ...
        <div class="nav-item nav-item-features nav-item-products" data-mega>
          <button type="button" class="nav-link" aria-expanded="false" aria-haspopup="true">Product <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="6 9 12 15 18 9"/></svg></button>
          ...
          <script>
            (function() {
              ...
              function setupProdocuMob() {
                var mobChanBtns = document.querySelectorAll(".prodocu-mob-channel-btn");
                if (!mobChanBtns.length) return;
                mobChanBtns.forEach(function(btn) {
                  btn.addEventListener("click", function(e) {
                    e.preventDefault();
                    e.stopPropagation();
                    var chan = this.getAttribute("data-mob-chan");
                    var panelId = "prodocu-mob-" + (chan === "whatsapp" ? "wa" : chan === "instagram" ? "ig" : chan === "telegram" ? "tg" : "fb");
                    var panel = document.getElementById(panelId);
                    var isOpen = this.classList.contains("active");

                    mobChanBtns.forEach(function(b) {
                      b.classList.remove("active");
                      var c = b.getAttribute("data-mob-chan");
                      var p = document.getElementById("prodocu-mob-" + (c === "whatsapp" ? "wa" : c === "instagram" ? "ig" : c === "telegram" ? "tg" : "fb"));
              ...
              function initAllProdocu() {
                setupProdocuMega();
                setupProdocuMob();
              }
              if (document.readyState === "loading") {
                document.addEventListener("DOMContentLoaded", initAllProdocu);
              } else {
                initAllProdocu();
              }
            })();
